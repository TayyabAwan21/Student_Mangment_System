...
class studentManagment {
    ...
    enrollStudent(name:string){
        const StuId= this.genId()
        let new_stud:Student={
            name,
            StuId,
            enroll_courses:[],
            balance:0
    ...
    addSubjects(StuId:number,Courses:Course[]){
         ...
         if(!student){
            ...
            const Fee:number =Courses.reduce((total,subject)=> total+ subject.fee,0)
  ...
  payFee(StuId:number,amount:number){
    const student=this.findId(StuId)
     if(student){
        ...
        else{console.log("Fee is not paid")}
}
else {console.log("Student does not exist")}
}

VeiwStatus(StuId:number){
    const student=this.findId(StuId)
     if(student){
